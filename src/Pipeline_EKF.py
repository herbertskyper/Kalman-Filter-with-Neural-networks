...
class Pipeline_EKF:
    ...
    def save(self):
        torch.save(self.model, self.PipelineName)
    ...
    def NNTest_RealTime(self, SysModel, test_input, load_model_path,T_test):
        '''用于实时更新的预测'''
        # Load model
        self.model = torch.load(load_model_path, map_location=self.device)
        ...
        self.N_T = 1
        SysModel.T_test=T_test
        # SysModel.T_test = test_input.size()[-1]
        # self.MSE_test_linear_arr = torch.zeros([self.N_T])
        x_out_test = torch.zeros([1, SysModel.m,SysModel.T_test]).to(self.device)
        ...
        # x_out_test[:,:, 0] = torch.squeeze(self.model(torch.unsqueeze(test_input[:,:, 0],2)))
    ...
    def PlotTrain_KF(self, MSE_KF_linear_arr, MSE_KF_dB_avg):
        '''Plot the KNet training results and the comparison with the KF'''
